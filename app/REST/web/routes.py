...
from app.REST.service.product_validators import ValidationError, ResourceNotFoundError, ConflictError
...
def patch_product_endpoint(id: int, payload: ProductUpdate, db: Session = Depends(get_db)):
    try:
        product = patch_product(db, id, payload)
        if product is None:
            raise HTTPException(status_code=404, detail="Product not found")
        return product
    except ValidationError as e:
